...
class Application:
    ...
    def __init__(self):
        ...
        # ── Theme colors ──
        self.BG       = "#1a1a2e"   # dark navy background
        self.BG2      = "#16213e"   # slightly lighter panel bg
        self.ACCENT   = "#0f3460"   # accent blue
        self.HIGHLIGHT = "#e94560"  # vibrant red-pink accent
        self.FG       = "#eaeaea"   # light text
        self.FG_DIM   = "#a0a0b0"   # dimmed text
        self.BTN_BG   = "#0f3460"   # button background
        self.BTN_FG   = "#ffffff"   # button text
        self.BTN_HOVER= "#e94560"   # button hover
        self.CARD_BG  = "#16213e"   # card background
        self.FONT_TITLE = ("Segoe UI", 22, "bold")
        self.FONT_LABEL = ("Segoe UI", 18, "bold")
        self.FONT_VALUE = ("Segoe UI", 22)
        self.FONT_BTN   = ("Segoe UI", 14, "bold")
        self.FONT_SUGGEST = ("Segoe UI", 13)
        ...
        # ── Title bar ──
        title_frame = tk.Frame(self.root, bg=self.HIGHLIGHT, height=60)
        title_frame.place(x=0, y=0, relwidth=1, height=60)

        self.T = tk.Label(title_frame, bg=self.HIGHLIGHT, fg="#ffffff")
        self.T.place(relx=0.5, rely=0.5, anchor="center")
        self.T.config(text="✋  Sign Language To Text & Speech  ✋", font=("Segoe UI", 24, "bold"))

        # ── Camera feed panel ──
        cam_frame = tk.Frame(self.root, bg=self.ACCENT, bd=0, highlightbackground=self.ACCENT, highlightthickness=3)
        cam_frame.place(x=30, y=75, width=496, height=456)
        cam_label = tk.Label(cam_frame, text="CAMERA FEED", bg=self.ACCENT, fg=self.FG_DIM, font=("Segoe UI", 9, "bold"))
        cam_label.place(x=5, y=2)
        self.panel = tk.Label(cam_frame, bg="#000000")
        self.panel.place(x=8, y=24, width=480, height=424)

        # ── Skeleton panel ──
        skel_frame = tk.Frame(self.root, bg=self.ACCENT, bd=0, highlightbackground=self.ACCENT, highlightthickness=3)
        skel_frame.place(x=550, y=75, width=416, height=456)
        skel_label = tk.Label(skel_frame, text="HAND SKELETON", bg=self.ACCENT, fg=self.FG_DIM, font=("Segoe UI", 9, "bold"))
        skel_label.place(x=5, y=2)
        self.panel2 = tk.Label(skel_frame, bg="#000000")
        self.panel2.place(x=8, y=24, width=400, height=424)

        # ── Right side info panel ──
        info_frame = tk.Frame(self.root, bg=self.CARD_BG, bd=0, highlightbackground=self.ACCENT, highlightthickness=2)
        info_frame.place(x=990, y=75, width=430, height=456)

        # Character display
        tk.Label(info_frame, text="DETECTED CHARACTER", bg=self.CARD_BG, fg=self.FG_DIM,
                 font=("Segoe UI", 11, "bold")).place(x=20, y=15)
        char_box = tk.Frame(info_frame, bg=self.BG, highlightbackground=self.HIGHLIGHT, highlightthickness=2)
        char_box.place(x=20, y=45, width=390, height=90)
        self.panel3 = tk.Label(char_box, bg=self.BG, fg=self.HIGHLIGHT, anchor="center")
        self.panel3.place(relx=0.5, rely=0.5, anchor="center")
        self.panel3.config(font=("Segoe UI", 48, "bold"))

        self.T1 = tk.Label(self.root)  # hidden, kept for compatibility
        self.T1.place_forget()

        # Sentence display
        tk.Label(info_frame, text="SENTENCE", bg=self.CARD_BG, fg=self.FG_DIM,
                 font=("Segoe UI", 11, "bold")).place(x=20, y=155)
        sentence_box = tk.Frame(info_frame, bg=self.BG, highlightbackground=self.ACCENT, highlightthickness=2)
        sentence_box.place(x=20, y=185, width=390, height=100)
        self.panel5 = tk.Label(sentence_box, bg=self.BG, fg=self.FG, anchor="nw", justify="left")
        self.panel5.place(x=10, y=10, width=370, height=80)
        self.panel5.config(font=("Segoe UI", 16), wraplength=360)

        self.T3 = tk.Label(self.root)  # hidden, kept for compatibility
        self.T3.place_forget()

        # Action buttons in info panel
        btn_y = 310
        self.speak = tk.Button(info_frame, text="🔊  Speak", font=self.FONT_BTN,
                               bg=self.HIGHLIGHT, fg=self.BTN_FG, activebackground="#c73650",
                               activeforeground="#ffffff", bd=0, cursor="hand2",
                               padx=20, pady=10, command=self.speak_fun)
        self.speak.place(x=20, y=btn_y, width=185, height=50)

        self.clear = tk.Button(info_frame, text="✕  Clear", font=self.FONT_BTN,
                               bg=self.ACCENT, fg=self.BTN_FG, activebackground="#1a4a80",
                               activeforeground="#ffffff", bd=0, cursor="hand2",
                               padx=20, pady=10, command=self.clear_fun)
        self.clear.place(x=225, y=btn_y, width=185, height=50)

        # Status indicator
        self.status_label = tk.Label(info_frame, text="●  Live", bg=self.CARD_BG, fg="#00e676",
                                     font=("Segoe UI", 12, "bold"))
        self.status_label.place(x=20, y=400)

        # ── Bottom bar: Suggestions ──
        bottom_frame = tk.Frame(self.root, bg=self.BG2, highlightbackground=self.ACCENT, highlightthickness=1)
        bottom_frame.place(x=0, y=550, relwidth=1, height=260)

        self.T4 = tk.Label(bottom_frame, bg=self.BG2, fg=self.HIGHLIGHT)
        self.T4.place(x=30, y=15)
        self.T4.config(text="💡 Suggestions", font=("Segoe UI", 16, "bold"))

        suggest_btn_style = dict(
            font=self.FONT_SUGGEST, bg=self.BTN_BG, fg=self.BTN_FG,
            activebackground=self.HIGHLIGHT, activeforeground="#ffffff",
            bd=0, cursor="hand2", padx=15, pady=8
        )
        self.b1 = tk.Button(bottom_frame, **suggest_btn_style)
        self.b1.place(x=30, y=60, width=320, height=45)

        self.b2 = tk.Button(bottom_frame, **suggest_btn_style)
        self.b2.place(x=370, y=60, width=320, height=45)

        self.b3 = tk.Button(bottom_frame, **suggest_btn_style)
        self.b3.place(x=710, y=60, width=320, height=45)

        self.b4 = tk.Button(bottom_frame, **suggest_btn_style)
        self.b4.place(x=1050, y=60, width=320, height=45)

        # ── Footer ──
        footer = tk.Label(bottom_frame, text="Sign Language Recognition System  •  Powered by CNN & MediaPipe",
                          bg=self.BG2, fg=self.FG_DIM, font=("Segoe UI", 10))
        footer.place(relx=0.5, y=130, anchor="center")
    ...
    def video_loop(self):
        try:
            ...
            if self.vs is None or not self.vs.isOpened():
                ...
                self.root.after(1000, self.video_loop)
                return
            ...
            if cv2image is not None and cv2image.size > 0:
                ...
                if hands[0]:
                    ...
                    # Create white background programmatically instead of loading file
            ...
            print(f"Exception in video_loop: {str(e)}")
            print(f"Traceback: {traceback.format_exc()}")
            try:
                # Try to continue with basic camera feed
                ok, frame = self.vs.read()
                if not ok or frame is None:
                    self.root.after(100, self.video_loop)
                    return
                ...
                # If all else fails, just continue the loop
                pass
        finally:
            # Schedule next frame with a small delay to prevent overwhelming the system
            try:
                if hasattr(self, 'root') and self.root and self.root.winfo_exists():
                    self.root.after(30, self.video_loop)
            except tk.TclError:
                # Window has been destroyed, stop the loop
                print("Window destroyed, stopping video loop")
                return
